...
with the negativity and the sum constraints, we discussed above. The $w_i$ weights are corresponding to the variance of the given gene measure in the same cell type. It aims to give less importance to the variant genes. EPIC also allows a cell type that is not a referenced in the signature matrix with an assumption that the non-referenced cell type is equal to 1- a sum of proportions of other cell types (Eq.\@ref(eq:tumorcoeff)). Authors interpret this non-referenced cell type as the tumor fraction:
